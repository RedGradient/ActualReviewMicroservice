...
def parse_google_reviews(url):
    ...
    for div in soup.find_all("div", class_="review"):
        try:
            ...
            rating = (
                float(div.find("meta", itemprop="ratingValue")["content"])
                if div.find("meta", itemprop="ratingValue")
                else None
            )
...
def parse_yandex_reviews(url):
    ...
    for div in soup.find_all("div", class_="comment__body"):
        try:
            ...
            rating = (
                int(div.find("span", class_="rating__value").text.strip())
                if div.find("span", class_="rating__value")
                else None
            )
            reviews.append((text, rating))
        except Exception as e:
